...
上述"先取锁、后裁决"成立的前提是所有事务运行在默认的 **READ COMMITTED**：
等锁结束后，锁后的每条语句按最新快照读取，因而能看到等待期间锁持有者
已提交的写入。若改用 REPEATABLE READ，快照在首条语句（即取锁本身）便
冻结，等锁者会看不到对方已提交的分片——重传与冲突被误判成重复写入
（唯一键冲突），已齐全的批次被误判成 `INCOMPLETE`。
